...
//  ********** Library utilslib **************
// ********** Code for top level **************
//  ********** Library gravityrandom **************
// ********** Code for gravityrandom **************
function gravityrandom() {

}
gravityrandom.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var particles = new Array();
  var numParticles = (30);
  for (var size, particle, i = (0);
   $lt(i, numParticles); i = $add(i, (1))) {
    size = Math.random() * (25) + (5);
    particle = new Ball.withRadius$ctor(size);
    particle.set$x(Math.random() * canvas.get$width());
    particle.set$y(Math.random() * canvas.get$height());
    particle.set$mass(size);
  ...
  function gravitate(partA, partB) {
    var dx = $sub(partB.get$x(), partA.get$x()), dy = $sub(partB.get$y(), partA.get$y()), distSQ = $mul(dx, dx) + $mul(dy, dy), dist = Math.sqrt(distSQ), force = $div(partA.get$mass() * partB.get$mass(), distSQ), ax = $div($mul(force, dx), dist), ay = $div($mul(force, dy), dist);
    partA.set$vx(partA.get$vx() + ($div(ax, partA.get$mass())));
    partA.set$vy(partA.get$vy() + ($div(ay, partA.get$mass())));
    partB.set$vx(partB.get$vx() - ($div(ax, partB.get$mass())));
    partB.set$vy(partB.get$vy() - ($div(ay, partB.get$mass())));
  }
  ;
  function move(partA, i) {
    partA.set$x($add(partA.get$x(), partA.get$vx()));
    partA.set$y($add(partA.get$y(), partA.get$vy()));
    for (var partB, j = $add(i, (1));
     $lt(j, numParticles); j = $add(j, (1))) {
      partB = particles.$index(j);
  ...
  function drawFrame(t) {
    ...
    var i = (0);
    particles.forEach$1((function (p) {
      move(p, i);
      i = $add(i, (1));
    })
    );
    particles.forEach$1(draw);
  }
  ;
  drawFrame((0));
}
