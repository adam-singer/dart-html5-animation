...
// ********** Code for top level **************
//  ********** Library nodegardenlines **************
// ********** Code for nodegardenlines **************
function nodegardenlines() {

}
nodegardenlines.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var particles = new Array();
  var numParticles = (30), minDist = (100), springAmount = (0.001);
  for (var particle, i = (0);
   $lt(i, numParticles); i = $add(i, (1))) {
    particle = new Ball.withRadiusAndColor$ctor((5), "#ffffff");
  ...
  function spring(partA, partB) {
    ...
    if ($lt(dist, minDist)) {
      ...
      partA.set$vx($add(partA.get$vx(), ax));
      partA.set$vy($add(partA.get$vy(), ay));
      partB.set$vx($sub(partB.get$vx(), ax));
      partB.set$vy($sub(partB.get$vy(), ay));
    }
  }
  ;
