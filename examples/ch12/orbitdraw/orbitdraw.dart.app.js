...
orbitdraw.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var particles = new Array();
  var numParticles = (2);
  var sun = new Ball.withRadiusAndColor$ctor((100), "#ffff00"), planet = new Ball.withRadiusAndColor$ctor((1), "#000000");
  sun.set$x(canvas.get$width() / (2));
  sun.set$y(canvas.get$height() / (2));
  sun.set$mass((10000));
  particles.add(sun);
  planet.set$x(canvas.get$width() / (2) + (200));
  planet.set$y(canvas.get$height() / (2));
  planet.set$vy((7));
  planet.set$mass((1));
