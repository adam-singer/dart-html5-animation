...
// ********** Code for top level **************
//  ********** Library nodegardenmass **************
// ********** Code for nodegardenmass **************
function nodegardenmass() {

}
nodegardenmass.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var particles = new Array();
  var numParticles = (30), minDist = (100), springAmount = (0.0005);
  for (var size, particle, i = (0);
   $lt(i, numParticles); i = $add(i, (1))) {
    size = Math.random() * (10) + (2);
    particle = new Ball.withRadiusAndColor$ctor(size, "#ffffff");
    particle.set$x(Math.random() * canvas.get$width());
    particle.set$y(Math.random() * canvas.get$height());
    particle.set$vx(Math.random() * (6) - (3));
    particle.set$vy(Math.random() * (6) - (3));
    particle.set$mass(size);
    particles.add(particle);
  }
  function spring(partA, partB) {
    var dx = $sub(partB.get$x(), partA.get$x()), dy = $sub(partB.get$y(), partA.get$y()), dist = Math.sqrt($mul(dx, dx) + $mul(dy, dy));
    if ($lt(dist, minDist)) {
      var alpha = (1) - $div(dist, minDist);
      context.set$strokeStyle(Utils.colorToRGB("#ffffff", alpha));
      context.beginPath();
      context.moveTo$2(partA.get$x(), partA.get$y());
      context.lineTo(partB.get$x(), partB.get$y());
      context.stroke();
      var ax = $mul(dx, springAmount), ay = $mul(dy, springAmount);
  ...
  function move(partA, i) {
    ...
    if ($gt(partA.get$x(), canvas.get$width())) {
      partA.set$x((0));
    }
    else if ($lt(partA.get$x(), (0))) {
      partA.set$x(canvas.get$width());
    }
    if ($gt(partA.get$y(), canvas.get$height())) {
      partA.set$y((0));
    }
    else if ($lt(partA.get$y(), (0))) {
      partA.set$y(canvas.get$height());
    }
