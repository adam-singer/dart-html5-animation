...
// ********** Code for top level **************
//  ********** Library mousedistance **************
// ********** Code for mousedistance **************
function mousedistance() {

}
mousedistance.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var log = html_get$document().query("#log");
  var mouse = Utils.captureMouse(canvas);
  var x = canvas.get$width() / (2);
  var y = canvas.get$height() / (2);
  ...
  function drawFrame(t) {
    ...
    var dx = $sub(rect.get$x(), mouse.get$x());
    var dy = $sub(rect.get$y(), mouse.get$y());
    var dist = Math.sqrt($mul(dx, dx) + $mul(dy, dy));
    context.set$fillStyle("#000000");
    context.fillRect($sub(rect.get$x(), (2)), $sub(rect.get$y(), (2)), (4), (4));
    context.beginPath();
    context.moveTo$2(rect.get$x(), rect.get$y());
    context.lineTo(mouse.get$x(), mouse.get$y());
    context.closePath();
    context.stroke();
    log.set$value($add("distance: ", dist));
  }
  ;
  drawFrame((0));
}
// ********** Code for rectLocal **************
function rectLocal(x, y) {
  this.y = y;
  this.x = x;
}
rectLocal.prototype.get$x = function() { return this.x; };
rectLocal.prototype.set$x = function(value) { return this.x = value; };
rectLocal.prototype.get$y = function() { return this.y; };
rectLocal.prototype.set$y = function(value) { return this.y = value; };
// ********** Code for top level **************
function main() {
