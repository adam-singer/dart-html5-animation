...
wave2.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var angle = (0);
  var range = (50);
  var centerY = canvas.get$height() / (2);
  var xspeed = (1);
  var yspeed = (0.05);
  var xpos = (0);
  var ypos = centerY;
  ...
  function drawFrame(t) {
    ...
    context.beginPath();
    context.moveTo$2(xpos, ypos);
    xpos = $add(xpos, xspeed);
    angle = $add(angle, yspeed);
    ypos = $add(centerY, $mul(Math.sin(angle), range));
