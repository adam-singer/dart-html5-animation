...
//  ********** Library utilslib **************
// ********** Code for top level **************
//  ********** Library multibilliard1 **************
// ********** Code for multibilliard1 **************
function multibilliard1() {

}
multibilliard1.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var balls = new Array();
  var numBalls = (8), bounce = (-1);
  for (var ball, i = (0);
   $lt(i, numBalls); i = $add(i, (1))) {
    var radius = Math.random() * (20) + (15);
    ball = new Ball.withRadius$ctor(radius);
    ball.set$mass(radius);
    ball.set$x($mul(i, (100)));
    ball.set$y($mul(i, (50)));
  ...
  function checkCollision(ball0, ball1) {
    ...
    if ($lt(dist, ball0.get$radius() + ball1.get$radius())) {
      ...
      pos0.set$x($add(pos0.get$x(), vel0.get$x()));
      pos1.set$x($add(pos1.get$x(), vel1.get$x()));
