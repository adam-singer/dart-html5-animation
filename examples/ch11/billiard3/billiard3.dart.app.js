...
billiard3.prototype.run = function() {
  ...
  function checkCollision() {
    ...
    if ($lt(dist, ball0.get$radius() + ball1.get$radius())) {
      var angle = Math.atan2(dy, dx), sin = Math.sin(angle), cos = Math.cos(angle), x0 = (0), y0 = (0), x1 = $mul(dx, cos) + $mul(dy, sin), y1 = $mul(dy, cos) - $mul(dx, sin), vx0 = $mul(ball0.get$vx(), cos) + $mul(ball0.get$vy(), sin), vy0 = $mul(ball0.get$vy(), cos) - $mul(ball0.get$vx(), sin), vx1 = $mul(ball1.get$vx(), cos) + $mul(ball1.get$vy(), sin), vy1 = $mul(ball1.get$vy(), cos) - $mul(ball1.get$vx(), sin), vxTotal = $sub(vx0, vx1);
      vx0 = ($mul((ball0.get$mass() - ball1.get$mass()), vx0) + $mul((2) * ball1.get$mass(), vx1)) / (ball0.get$mass() + ball1.get$mass());
      vx1 = $add(vxTotal, vx0);
      x0 = $add(x0, vx0);
      x1 = $add(x1, vx1);
      var x0Final = $mul(x0, cos) - $mul(y0, sin), y0Final = $mul(y0, cos) + $mul(x0, sin), x1Final = $mul(x1, cos) - $mul(y1, sin), y1Final = $mul(y1, cos) + $mul(x1, sin);
      ball1.set$x($add(ball0.get$x(), x1Final));
      ball1.set$y($add(ball0.get$y(), y1Final));
      ball0.set$x($add(ball0.get$x(), x0Final));
      ball0.set$y($add(ball0.get$y(), y0Final));
      ball0.set$vx($mul(vx0, cos) - $mul(vy0, sin));
      ball0.set$vy($mul(vy0, cos) + $mul(vx0, sin));
      ball1.set$vx($mul(vx1, cos) - $mul(vy1, sin));
      ball1.set$vy($mul(vy1, cos) + $mul(vx1, sin));
    }
  }
  ;
