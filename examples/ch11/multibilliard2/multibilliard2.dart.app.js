...
// ********** Code for top level **************
//  ********** Library multibilliard2 **************
// ********** Code for multibilliard2 **************
function multibilliard2() {

}
multibilliard2.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var balls = new Array();
  var numBalls = (15), bounce = (-1);
  for (var radius, ball, i = (0);
   $lt(i, numBalls); i = $add(i, (1))) {
    radius = Math.random() * (20) + (15);
    var color = Utils.parseColor(Math.random() * (16777215), false);
    ball = new Ball.withRadiusAndColor$ctor(radius, color);
    ball.set$mass(radius);
    ball.set$x(Math.random() * canvas.get$width());
    ball.set$y(Math.random() * canvas.get$height());
    ball.set$vx(Math.random() * (10) - (5));
    ball.set$vy(Math.random() * (10) - (5));
    balls.add(ball);
  }
  function rotate(x, y, sin, cos, reverse) {
    return new point((reverse) ? ($mul(x, cos) + $mul(y, sin)) : ($mul(x, cos) - $mul(y, sin)), (reverse) ? ($mul(y, cos) - $mul(x, sin)) : ($mul(y, cos) + $mul(x, sin)));
  }
  ;
  function checkCollision(ball0, ball1) {
    var dx = $sub(ball1.get$x(), ball0.get$x()), dy = $sub(ball1.get$y(), ball0.get$y()), dist = Math.sqrt($mul(dx, dx) + $mul(dy, dy));
    if ($lt(dist, ball0.get$radius() + ball1.get$radius())) {
      var angle = Math.atan2(dy, dx), sin = Math.sin(angle), cos = Math.cos(angle), pos0 = new point((0), (0)), pos1 = rotate(dx, dy, sin, cos, true), vel0 = rotate(ball0.get$vx(), ball0.get$vy(), sin, cos, true), vel1 = rotate(ball1.get$vx(), ball1.get$vy(), sin, cos, true), vxTotal = $sub(vel0.get$x(), vel1.get$x());
      vel0.set$x(($mul((ball0.get$mass() - ball1.get$mass()), vel0.get$x()) + $mul((2) * ball1.get$mass(), vel1.get$x())) / (ball0.get$mass() + ball1.get$mass()));
      vel1.set$x($add(vxTotal, vel0.get$x()));
      var absV = vel0.get$x().abs() + vel1.get$x().abs(), overlap = (ball0.get$radius() + ball1.get$radius()) - ($sub(pos0.get$x(), pos1.get$x())).abs();
      pos0.set$x($add(pos0.get$x(), ($mul($div(vel0.get$x(), absV), overlap))));
      pos1.set$x($add(pos1.get$x(), ($mul($div(vel1.get$x(), absV), overlap))));
      var pos0F = rotate(pos0.get$x(), pos0.get$y(), sin, cos, false), pos1F = rotate(pos1.get$x(), pos1.get$y(), sin, cos, false);
      ball1.set$x($add(ball0.get$x(), pos1F.get$x()));
      ball1.set$y($add(ball0.get$y(), pos1F.get$y()));
      ball0.set$x($add(ball0.get$x(), pos0F.get$x()));
      ball0.set$y($add(ball0.get$y(), pos0F.get$y()));
      var vel0F = rotate(vel0.get$x(), vel0.get$y(), sin, cos, false), vel1F = rotate(vel1.get$x(), vel1.get$y(), sin, cos, false);
      ball0.set$vx(vel0F.get$x());
      ball0.set$vy(vel0F.get$y());
      ball1.set$vx(vel1F.get$x());
      ball1.set$vy(vel1F.get$y());
    }
  }
  ;
  ...
  function drawFrame(t) {
    ...
    for (var ballA, i = (0), len = $sub(numBalls, (1));
     $lt(i, len); i = $add(i, (1))) {
      ballA = balls.$index(i);
      for (var ballB = null, j = $add(i, (1));
...
// ********** Code for Ball **************
Ball.withRadiusAndColor$ctor = function(radius, color) {
  this.radius = radius;
  this.color = color;
  this._defaultValues();
}
Ball.withRadiusAndColor$ctor.prototype = Ball.prototype;
function Ball() {}
...
Ball.prototype.get$mass = function() { return this.mass; };
Ball.prototype.set$mass = function(value) { return this.mass = value; };
