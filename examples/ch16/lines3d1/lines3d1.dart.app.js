...
lines3d1.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var mouse = Utils.captureMouse(canvas);
  var balls = [], numBalls = (15), fl = (250), vpX = canvas.get$width() / (2), vpY = canvas.get$height() / (2), angleX = (0), angleY = (0);
  for (var ball, i = (0);
   $lt(i, numBalls); i = $add(i, (1))) {
    ball = new Ball3d.withRadiusAndColor$ctor((5), "#000000");
  ...
  function rotateX(ball, angle) {
    var cos = Math.cos(angle), sin = Math.sin(angle), y1 = $mul(ball.get$ypos(), cos) - $mul(ball.get$zpos(), sin), z1 = $mul(ball.get$zpos(), cos) + $mul(ball.get$ypos(), sin);
    ball.set$ypos(y1);
    ball.set$zpos(z1);
  }
  function rotateY(ball, angle) {
    var cos = Math.cos(angle), sin = Math.sin(angle), x1 = $mul(ball.get$xpos(), cos) - $mul(ball.get$zpos(), sin), z1 = $mul(ball.get$zpos(), cos) + $mul(ball.get$xpos(), sin);
    ball.set$xpos(x1);
    ball.set$zpos(z1);
  }
  ...
  function move(ball, i) {
    ...
    if ((null == i ? null != ((0)) : i !== (0))) {
      context.lineTo(balls.$index(i).get$x(), balls.$index(i).get$y());
    }
  }
  function draw(ball) {
    if (ball.get$visible()) {
  ...
  function drawFrame(t) {
    ...
    context.beginPath();
    context.moveTo$2(balls.$index((0)).get$x(), balls.$index((0)).get$y());
    var i = (0);
...
// ********** Code for Ball3d **************
Ball3d.withRadiusAndColor$ctor = function(radius, color) {
  this.radius = radius;
  this.color = color;
  this._defaultValues();
}
Ball3d.withRadiusAndColor$ctor.prototype = Ball3d.prototype;
function Ball3d() {}
