...
rotatexy.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var mouse = Utils.captureMouse(canvas);
  var balls = [], numBalls = (50), fl = (250), vpX = canvas.get$width() / (2), vpY = canvas.get$height() / (2), angleX = (0), angleY = (0);
  for (var ball, i = (0);
   $lt(i, numBalls); i = $add(i, (1))) {
    ball = new Ball3d.withRadius$ctor((15));
    ball.set$xpos(Math.random() * (200) - (100));
    ball.set$ypos(Math.random() * (200) - (100));
    ball.set$zpos(Math.random() * (200) - (100));
    balls.add$1(ball);
  }
  function matrixMultiply(matrixA, matrixB) {
    var result = [];
    result.add$1($mul(matrixA.$index((0)), matrixB.$index((0)).$index((0))) + $mul(matrixA.$index((1)), matrixB.$index((1)).$index((0))) + $mul(matrixA.$index((2)), matrixB.$index((2)).$index((0))));
    result.add$1($mul(matrixA.$index((0)), matrixB.$index((0)).$index((1))) + $mul(matrixA.$index((1)), matrixB.$index((1)).$index((1))) + $mul(matrixA.$index((2)), matrixB.$index((2)).$index((1))));
    result.add$1($mul(matrixA.$index((0)), matrixB.$index((0)).$index((2))) + $mul(matrixA.$index((1)), matrixB.$index((1)).$index((2))) + $mul(matrixA.$index((2)), matrixB.$index((2)).$index((2))));
    return result;
  }
  function rotateX(ball, angle) {
    var position = [ball.get$xpos(), ball.get$ypos(), ball.get$zpos()], sin = Math.sin(angle), cos = Math.cos(angle), xRotMatrix = [];
    xRotMatrix.add$1([(1), (0), (0)]);
    xRotMatrix.add$1([(0), cos, sin]);
    xRotMatrix.add$1([(0), $negate(sin), cos]);
    var result = matrixMultiply(position, xRotMatrix);
    ball.set$xpos(result.$index((0)));
    ball.set$ypos(result.$index((1)));
    ball.set$zpos(result.$index((2)));
  }
  function rotateY(ball, angle) {
    var position = [ball.get$xpos(), ball.get$ypos(), ball.get$zpos()], sin = Math.sin(angle), cos = Math.cos(angle), yRotMatrix = [];
    yRotMatrix.add$1([cos, (0), sin]);
    yRotMatrix.add$1([(0), (1), (0)]);
    yRotMatrix.add$1([$negate(sin), (0), cos]);
  ...
  function drawFrame(t) {
    ...
    angleX = ($sub(mouse.get$y(), vpY)) * (0.001);
    angleY = ($sub(mouse.get$x(), vpX)) * (0.001);
