...
CanvasRenderingContext2DWrappingImplementation.prototype.transform = function(m11, m12, m21, m22, dx, dy) {
  this._ptr.transform(m11, m12, m21, m22, dx, dy);
  return;
}
...
skewxy.prototype.run = function() {
  ...
  function drawFrame(t) {
    ...
    var skewX = ($sub(mouse.get$x(), canvas.get$width() / (2))) * (0.01), skewY = ($sub(mouse.get$y(), canvas.get$height() / (2))) * (0.01), dx = canvas.get$width() / (2), dy = canvas.get$height() / (2);
    context.save();
    context.set$fillStyle("#ff0000");
    context.transform$6((1), skewY, skewX, (1), dx, dy);
