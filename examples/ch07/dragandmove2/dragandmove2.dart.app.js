...
// ********** Code for top level **************
//  ********** Library dragandmove2 **************
// ********** Code for dragandmove2 **************
function dragandmove2() {

}
dragandmove2.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var mouse = Utils.captureMouse(canvas);
  var ball = new Ball();
  var vx = Math.random() * (10) - (5), vy = (-10), bounce = (-0.7), gravity = (0.2), isMouseDown = false;
  ball.set$x(canvas.get$width() / (2));
  ball.set$y(canvas.get$height() / (2));
  function onMouseMove(event) {
    ball.set$x(mouse.get$x());
    ball.set$y(mouse.get$y());
  }
  ;
  function onMouseUp(event) {
    isMouseDown = false;
    canvas.get$on().get$mouseUp().remove(onMouseUp, false);
    canvas.get$on().get$mouseMove().remove(onMouseMove, false);
  }
  ;
  canvas.get$on().get$mouseDown().add((function (event) {
    if (Utils.containsPoint(ball.getBounds(), mouse.get$x(), mouse.get$y())) {
      isMouseDown = true;
      vx = vy = (0);
      canvas.get$on().get$mouseUp().add(onMouseUp, false);
      canvas.get$on().get$mouseMove().add(onMouseMove, false);
    }
  })
  , false);
  function checkBoundaries() {
    var left = (0), right = canvas.get$width(), top = (0), bottom = canvas.get$height();
    vy = $add(vy, gravity);
    ball.set$x($add(ball.get$x(), vx));
    ball.set$y($add(ball.get$y(), vy));
    if ($gt($add(ball.get$x(), ball.get$radius()), right)) {
      ball.set$x($sub(right, ball.get$radius()));
      vx = $mul(vx, bounce);
    }
    else if ($lt($sub(ball.get$x(), ball.get$radius()), left)) {
      ball.set$x($add(left, ball.get$radius()));
      vx = $mul(vx, bounce);
    }
    if ($gt($add(ball.get$y(), ball.get$radius()), bottom)) {
      ball.set$y($sub(bottom, ball.get$radius()));
      vy = $mul(vy, bounce);
    }
    else if ($lt($sub(ball.get$y(), ball.get$radius()), top)) {
      ball.set$y($add(top, ball.get$radius()));
      vy = $mul(vy, bounce);
    }
  }
