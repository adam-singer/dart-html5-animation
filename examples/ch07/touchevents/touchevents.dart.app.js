...
WindowEventsImplementation.prototype.get$touchMove = function() {
  return this._get("touchmove");
}
...
// ********** Code for top level **************
//  ********** Library touchevents **************
// ********** Code for touchevents **************
function touchevents() {

}
touchevents.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var _touch = Utils.captureMouse(canvas);
  var log = html_get$document().query("#log");
  var ball = new Ball();
  ball.set$x(canvas.get$width() / (2));
  ball.set$y(canvas.get$height() / (2));
  ball.draw(context);
  canvas.get$on().get$touchStart().add((function (event) {
    if (Utils.containsPoint(ball.getBounds(), _touch.get$x(), _touch.get$y())) {
      log.set$value("in ball: touchstart");
    }
    else {
      log.set$value("canvas: touchstart");
    }
  })
  , false);
  canvas.get$on().get$touchEnd().add((function (event) {
    event.preventDefault();
    log.set$value("canvas: touchend");
  })
  , false);
  canvas.get$on().get$touchMove().add((function (event) {
    event.preventDefault();
    if (Utils.containsPoint(ball.getBounds(), _touch.get$x(), _touch.get$y())) {
      log.set$value("in ball: touchmove");
    }
    else {
      log.set$value("canvas: touchmove");
    }
  })
  , false);
}
