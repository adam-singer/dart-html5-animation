...
throwing.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var mouse = Utils.captureMouse(canvas);
  var ball = new Ball();
  var vx = Math.random() * (10) - (5), vy = (-10), bounce = (-0.7), gravity = (0.2), isMouseDown = false, oldX, oldY;
  ...
  function trackVelocity() {
    vx = $sub(ball.get$x(), oldX);
    vy = $sub(ball.get$y(), oldY);
    oldX = ball.get$x();
    oldY = ball.get$y();
  }
  ;
