...
$dynamic("set$id").HTMLElement = function(value) {
  this.id = value;
}
...
$dynamic("set$id").TextTrackCue = function(value) {
  this.id = value;
}
...
removal.prototype.run = function() {
  ...
  for (var i = (0);
   $lt(i, numBalls); i = $add(i, (1))) {
    ...
    ball.set$vx(Math.random() * (2) - (1));
    ball.set$vy(Math.random() * (2) - (1));
    balls.add(ball);
  }
  function draw(ball, pos) {
    ball.set$x($add(ball.get$x(), ball.get$vx()));
    ball.set$y($add(ball.get$y(), ball.get$vy()));
    if ($sub(ball.get$x(), ball.get$radius()) > canvas.get$width() || $lt($add(ball.get$x(), ball.get$radius()), (0)) || $sub(ball.get$y(), ball.get$radius()) > canvas.get$height() || $lt($add(ball.get$y(), ball.get$radius()), (0))) {
      balls.removeRange(pos, (1));
      if (balls.get$length() > (0)) {
        log.set$value($add("Removed ", ball.get$id()));
      }
      else {
        log.set$value("All gone!");
      }
    }
    ball.draw(context);
  }
  ...
  function drawFrame(t) {
    ...
    var i = balls.get$length();
    while (($0 = i, i = $sub(i, (1)), $0)) {
      draw(balls.$index(i), i);
    }
  }
  ;
  drawFrame((0));
...
Ball.prototype.get$id = function() { return this.id; };
Ball.prototype.set$id = function(value) { return this.id = value; };
