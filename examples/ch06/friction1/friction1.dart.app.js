...
friction1.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var ball = new Ball();
  var vx = Math.random() * (10) - (5), vy = Math.random() * (10) - (5), friction = (0.1);
  ...
  function drawFrame(t) {
    ...
    var speed = Math.sqrt($mul(vx, vx) + $mul(vy, vy)), angle = Math.atan2(vy, vx);
    if ($gt(speed, friction)) {
      speed = $sub(speed, friction);
    }
    else {
      speed = (0);
    }
    vx = $mul(Math.cos(angle), speed);
    vy = $mul(Math.sin(angle), speed);
