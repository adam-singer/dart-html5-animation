...
//  ********** Library utilslib **************
// ********** Code for top level **************
//  ********** Library shipsimfriction **************
// ********** Code for shipsimfriction **************
function shipsimfriction() {

}
shipsimfriction.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var ship = new Ship();
  var vr = (0), vx = (0), vy = (0), thrust = (0), friction = (0.97);
  ship.set$x(canvas.get$width() / (2));
  ship.set$y(canvas.get$height() / (2));
  ...
  function drawFrame(t) {
    ...
    ship.set$rotation(ship.get$rotation() + ($mul(vr, (3.141592653589793)) / (180)));
    var angle = ship.get$rotation(), ax = $mul(Math.cos(angle), thrust), ay = $mul(Math.sin(angle), thrust), left = (0), right = canvas.get$width(), top = (0), bottom = canvas.get$height();
    vx = $add(vx, ax);
    vy = $add(vy, ay);
    vx = $mul(vx, friction);
    vy = $mul(vy, friction);
    ship.set$x($add(ship.get$x(), vx));
    ship.set$y($add(ship.get$y(), vy));
    if ($gt($sub(ship.get$x(), $div(ship.get$width(), (2))), right)) {
      ship.set$x($sub(left, $div(ship.get$width(), (2))));
    }
    else if ($lt($add(ship.get$x(), $div(ship.get$width(), (2))), left)) {
      ship.set$x($add(right, $div(ship.get$width(), (2))));
    }
    if ($gt($sub(ship.get$y(), $div(ship.get$height(), (2))), bottom)) {
      ship.set$y($sub(top, $div(ship.get$height(), (2))));
    }
    else if ($lt(ship.get$y(), $sub(top, $div(ship.get$height(), (2))))) {
      ship.set$y($add(bottom, $div(ship.get$height(), (2))));
    }
    ship.draw(context);
  }
  ;
...
Ship.prototype.get$x = function() { return this.x; };
Ship.prototype.set$x = function(value) { return this.x = value; };
Ship.prototype.get$y = function() { return this.y; };
Ship.prototype.set$y = function(value) { return this.y = value; };
Ship.prototype.get$width = function() { return this.width; };
Ship.prototype.set$width = function(value) { return this.width = value; };
Ship.prototype.get$height = function() { return this.height; };
Ship.prototype.set$height = function(value) { return this.height = value; };
Ship.prototype.get$rotation = function() { return this.rotation; };
Ship.prototype.set$rotation = function(value) { return this.rotation = value; };
Ship.prototype.get$showFlame = function() { return this.showFlame; };
Ship.prototype.set$showFlame = function(value) { return this.showFlame = value; };
