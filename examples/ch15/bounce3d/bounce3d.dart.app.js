...
bounce3d.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var ball = new Ball();
  var xpos = (0), ypos = (0), zpos = (0), vx = Math.random() * (10) - (5), vy = Math.random() * (10) - (5), vz = Math.random() * (10) - (5), fl = (250), vpX = canvas.get$width() / (2), vpY = canvas.get$height() / (2), top = (-100), bottom = (100), left = (-100), right = (100), back = (100), front = (-100);
  function drawFrame(t) {
    ...
    ypos = $add(ypos, vy);
    zpos = $add(zpos, vz);
    if ($gt($add(xpos, ball.get$radius()), right)) {
      xpos = $sub(right, ball.get$radius());
      vx = $mul(vx, (-1));
    }
    else if ($lt($sub(xpos, ball.get$radius()), left)) {
      xpos = $add(left, ball.get$radius());
      vx = $mul(vx, (-1));
    }
    if ($gt($add(ypos, ball.get$radius()), bottom)) {
      ypos = $sub(bottom, ball.get$radius());
      vy = $mul(vy, (-1));
    }
    else if ($lt($sub(ypos, ball.get$radius()), top)) {
      ypos = $add(top, ball.get$radius());
      vy = $mul(vy, (-1));
    }
    if ($gt($add(zpos, ball.get$radius()), back)) {
      zpos = $sub(back, ball.get$radius());
      vz = $mul(vz, (-1));
    }
    else if ($lt($sub(zpos, ball.get$radius()), front)) {
      zpos = $add(front, ball.get$radius());
      vz = $mul(vz, (-1));
    }
