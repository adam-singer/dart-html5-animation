...
// ********** Code for top level **************
//  ********** Library bouncyballs **************
// ********** Code for bouncyballs **************
function bouncyballs() {

}
bouncyballs.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var balls = [], numBalls = (150), fl = (250), vpX = canvas.get$width() / (2), vpY = canvas.get$height() / (2), gravity = (0.2), floor = (200), bounce = (-0.6);
  for (var ball, i = (0);
   $lt(i, numBalls); i = $add(i, (1))) {
    var color = Utils.parseColor(Math.random() * (16777215), false);
    ball = new Ball3d.withRadiusAndColor$ctor((3), color);
    ball.set$ypos((-100));
    ball.set$vx(Math.random() * (6) - (3));
    ball.set$vy(Math.random() * (6) - (3));
    ball.set$vz(Math.random() * (6) - (3));
    balls.add$1(ball);
  }
  function move(ball) {
    var $0;
