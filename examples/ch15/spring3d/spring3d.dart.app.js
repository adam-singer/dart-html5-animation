...
spring3d.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var ball = new Ball3d(), tx = Math.random() * (500) - (250), ty = Math.random() * (500) - (250), tz = Math.random() * (500), spring = (0.1), friction = (0.94), fl = (250), vpX = canvas.get$width() / (2), vpY = canvas.get$height() / (2);
  html_get$window().get$on().get$mouseDown().add((function (event) {
  ...
  function drawFrame(t) {
    ...
    var dx = $sub(tx, ball.get$xpos()), dy = $sub(ty, ball.get$ypos()), dz = $sub(tz, ball.get$zpos());
    ball.set$vx(ball.get$vx() + ($mul(dx, spring)));
    ball.set$vy(ball.get$vy() + ($mul(dy, spring)));
    ball.set$vz(ball.get$vz() + ($mul(dz, spring)));
    ...
    ball.set$vz($mul(ball.get$vz(), friction));
