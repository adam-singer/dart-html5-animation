...
easing3d.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var ball = new Ball3d(), tx = Math.random() * (500) - (250), ty = Math.random() * (500) - (250), tz = Math.random() * (500), easing = (0.1), fl = (250), vpX = canvas.get$width() / (2), vpY = canvas.get$height() / (2);
  function drawFrame(t) {
    ...
    var dx = $sub(tx, ball.get$xpos()), dy = $sub(ty, ball.get$ypos()), dz = $sub(tz, ball.get$zpos()), dist = Math.sqrt($mul(dx, dx) + $mul(dy, dy) + $mul(dz, dz));
    ball.set$xpos(ball.get$xpos() + ($mul(dx, easing)));
    ball.set$ypos(ball.get$ypos() + ($mul(dy, easing)));
    ball.set$zpos(ball.get$zpos() + ($mul(dz, easing)));
    if ($lt(dist, (1))) {
...
(function(){
  ...
  var v1/*HTMLCollection*/ = 'HTMLCollection|HTMLOptionsCollection|HTMLPropertiesCollection';
