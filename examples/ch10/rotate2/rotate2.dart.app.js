...
rotate2.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var ball = new Ball();
  var vr = (0.05), cos = Math.cos(vr), sin = Math.sin(vr), centerX = canvas.get$width() / (2), centerY = canvas.get$height() / (2);
  ...
  function drawFrame(t) {
    ...
    var x1 = $sub(ball.get$x(), centerX), y1 = $sub(ball.get$y(), centerY), x2 = $mul(cos, x1) - $mul(sin, y1), y2 = $mul(cos, y1) + $mul(sin, x1);
    ball.set$x($add(centerX, x2));
    ball.set$y($add(centerY, y2));
    ball.draw(context);
  }
  ;
  drawFrame((0));
