...
rotate1.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var ball = new Ball();
  var vr = (0.05), angle = (0), radius = (150), centerX = canvas.get$width() / (2), centerY = canvas.get$height() / (2);
  function drawFrame(t) {
    ...
    ball.set$x($add(centerX, $mul(Math.cos(angle), radius)));
    ball.set$y($add(centerY, $mul(Math.sin(angle), radius)));
    angle = $add(angle, vr);
    ball.draw(context);
  }
  ;
