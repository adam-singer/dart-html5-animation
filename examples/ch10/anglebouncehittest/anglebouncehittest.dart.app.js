...
Utils.intersects = function(rectA, rectB) {
  return !($lt($add(rectA.get$x(), rectA.get$width()), rectB.get$x()) || $lt($add(rectB.get$x(), rectB.get$width()), rectA.get$x()) || $lt($add(rectA.get$y(), rectA.get$height()), rectB.get$y()) || $lt($add(rectB.get$y(), rectB.get$height()), rectA.get$y()));
}
...
// ********** Code for top level **************
//  ********** Library anglebouncehittest **************
// ********** Code for anglebouncehittest **************
function anglebouncehittest() {

}
anglebouncehittest.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var mouse = Utils.captureMouse(canvas);
  var ball = new Ball();
  var line = new Line.withParameters$ctor((0), (0), (300), (0));
  ...
  function drawFrame(t) {
    ...
    if (Utils.intersects(ball.getBounds(), line.getBounds())) {
      ...
      if ($gt(y2, -ball.get$radius())) {
        var x2 = $mul(cos, x1) + $mul(sin, y1), vx1 = $mul(cos, ball.get$vx()) + $mul(sin, ball.get$vy()), vy1 = $mul(cos, ball.get$vy()) - $mul(sin, ball.get$vx());
        y2 = -ball.get$radius();
        vy1 = $mul(vy1, bounce);
