...
// ********** Code for top level **************
//  ********** Library multianglebounce **************
// ********** Code for multianglebounce **************
function multianglebounce() {

}
multianglebounce.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var mouse = Utils.captureMouse(canvas);
  var ball = new Ball.withRadius$ctor((20));
  var lines = new Array((5));
  var gravity = (0.2), bounce = (-0.6);
  ball.set$x((100));
  ball.set$y((50));
  lines.$setindex((0), new Line.withParameters$ctor((-50), (0), (50), (0)));
  lines.$index((0)).set$x((100));
  lines.$index((0)).set$y((100));
  lines.$index((0)).set$rotation((0.5235987755982988));
  lines.$setindex((1), new Line.withParameters$ctor((-50), (0), (50), (0)));
  lines.$index((1)).set$x((100));
  lines.$index((1)).set$y((200));
  lines.$index((1)).set$rotation((0.7853981633974483));
  lines.$setindex((2), new Line.withParameters$ctor((-50), (0), (50), (0)));
  lines.$index((2)).set$x((220));
  lines.$index((2)).set$y((150));
  lines.$index((2)).set$rotation((-0.3490658503988659));
  lines.$setindex((3), new Line.withParameters$ctor((-50), (0), (50), (0)));
  lines.$index((3)).set$x((150));
  lines.$index((3)).set$y((330));
  lines.$index((3)).set$rotation((0.17453292519943295));
  lines.$setindex((4), new Line.withParameters$ctor((-50), (0), (50), (0)));
  lines.$index((4)).set$x((230));
  lines.$index((4)).set$y((250));
  lines.$index((4)).set$rotation((-0.5235987755982988));
  function checkLine(line) {
    var bounds = line.getBounds();
    if ($gt($add(ball.get$x(), ball.get$radius()), bounds.get$x()) && $lt($sub(ball.get$x(), ball.get$radius()), $add(bounds.get$x(), bounds.get$width()))) {
      var cos = Math.cos(line.get$rotation()), sin = Math.sin(line.get$rotation()), x1 = $sub(ball.get$x(), line.get$x()), y1 = $sub(ball.get$y(), line.get$y()), y2 = $mul(cos, y1) - $mul(sin, x1), vy1 = $mul(cos, ball.get$vy()) - $mul(sin, ball.get$vx());
      if ($gt(y2, -ball.get$radius()) && $lt(y2, vy1)) {
        var x2 = $mul(cos, x1) + $mul(sin, y1), vx1 = $mul(cos, ball.get$vx()) + $mul(sin, ball.get$vy());
        y2 = -ball.get$radius();
        vy1 = $mul(vy1, bounce);
        x1 = $mul(cos, x2) - $mul(sin, y2);
        y1 = $mul(cos, y2) + $mul(sin, x2);
        ball.set$vx($mul(cos, vx1) - $mul(sin, vy1));
        ball.set$vy($mul(cos, vy1) + $mul(sin, vx1));
        ball.set$x($add(line.get$x(), x1));
        ball.set$y($add(line.get$y(), y1));
      }
    }
  }
  ;
  ...
  function drawFrame(t) {
    ...
    ball.set$vy($add(ball.get$vy(), gravity));
    ball.set$x($add(ball.get$x(), ball.get$vx()));
    ball.set$y($add(ball.get$y(), ball.get$vy()));
    if ($gt($add(ball.get$x(), ball.get$radius()), canvas.get$width())) {
      ball.set$x(canvas.get$width() - ball.get$radius());
      ball.set$vx($mul(ball.get$vx(), bounce));
    }
    else if ($sub(ball.get$x(), ball.get$radius()) < (0)) {
      ball.set$x(ball.get$radius());
      ball.set$vx($mul(ball.get$vx(), bounce));
    }
    if ($gt($add(ball.get$y(), ball.get$radius()), canvas.get$height())) {
      ball.set$y(canvas.get$height() - ball.get$radius());
      ball.set$vy($mul(ball.get$vy(), bounce));
    }
    else if ($sub(ball.get$y(), ball.get$radius()) < (0)) {
      ball.set$y(ball.get$radius());
      ball.set$vy($mul(ball.get$vy(), bounce));
    }
    lines.forEach$1(drawLine);
    ball.draw(context);
  }
  ;
  drawFrame((0));
...
// ********** Code for Ball **************
Ball.withRadius$ctor = function(radius) {
  this.radius = radius;
  this.color = "#ff0000";
  this._defaultValues();
}
Ball.withRadius$ctor.prototype = Ball.prototype;
function Ball() {}
...
Ball.prototype.get$vx = function() { return this.vx; };
Ball.prototype.set$vx = function(value) { return this.vx = value; };
Ball.prototype.get$vy = function() { return this.vy; };
Ball.prototype.set$vy = function(value) { return this.vy = value; };
Ball.prototype.get$radius = function() { return this.radius; };
Ball.prototype.set$radius = function(value) { return this.radius = value; };
Ball.prototype.get$rotation = function() { return this.rotation; };
Ball.prototype.set$rotation = function(value) { return this.rotation = value; };
...
Ball.prototype.getBounds = function() {
  return new localBound(this.x - this.radius, this.y - this.radius, this.radius * (2), this.radius * (2));
}
// ********** Code for localBound **************
...
// ********** Code for Line **************
Line.withParameters$ctor = function(x1, y1, x2, y2) {
  this.y1 = y1;
  this.x2 = x2;
  this.x1 = x1;
  this.y2 = y2;
...
Line.withParameters$ctor.prototype = Line.prototype;
function Line() {}
...
Line.prototype.get$rotation = function() { return this.rotation; };
Line.prototype.set$rotation = function(value) { return this.rotation = value; };
Line.prototype.get$lineWidth = function() { return this.lineWidth; };
Line.prototype.set$lineWidth = function(value) { return this.lineWidth = value; };
Line.prototype._defaultValues = function() {
  this.x = (0);
  this.y = (0);
  this.rotation = (0);
  this.scaleX = (1);
...
Line.prototype.draw = function(context) {
  ...
  context.set$lineWidth(this.lineWidth);
  context.beginPath();
  context.moveTo$2(this.x1, this.y1);
...
Line.prototype.getBounds = function() {
  var $0;
  if ((($0 = this.rotation) == null ? null == ((0)) : $0 === (0))) {
    var minX = Math.min(this.x1, this.x2), minY = Math.min(this.y1, this.y2), maxX = Math.max(this.x1, this.x2), maxY = Math.max(this.y1, this.y2);
    return new localBound($add(this.x, minX), $add(this.y, minY), $sub(maxX, minX), $sub(maxY, minY));
  }
  else {
    var sin = Math.sin(this.rotation), cos = Math.cos(this.rotation), x1r = $mul(cos, this.x1) + $mul(sin, this.y1), x2r = $mul(cos, this.x2) + $mul(sin, this.y2), y1r = $mul(cos, this.y1) + $mul(sin, this.x1), y2r = $mul(cos, this.y2) + $mul(sin, this.x2);
    return new localBound(this.x + Math.min(x1r, x2r), this.y + Math.min(y1r, y2r), Math.max(x1r, x2r) - Math.min(x1r, x2r), Math.max(y1r, y2r) - Math.min(y1r, y2r));
  }
}
// ********** Code for top level **************
function main() {
