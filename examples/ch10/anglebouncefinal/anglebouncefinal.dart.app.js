...
// ********** Code for top level **************
//  ********** Library anglebouncefinal **************
// ********** Code for anglebouncefinal **************
function anglebouncefinal() {

}
anglebouncefinal.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var mouse = Utils.captureMouse(canvas);
  var ball = new Ball();
  var line = new Line.withParameters$ctor((0), (0), (200), (0));
  ...
  function drawFrame(t) {
    ...
    line.set$rotation((($sub(canvas.get$width() / (2), mouse.get$x())) * (0.1)) * (3.141592653589793) / (180));
    ...
    if ($gt($add(ball.get$x(), ball.get$radius()), bounds.get$x()) && $lt($sub(ball.get$x(), ball.get$radius()), $add(bounds.get$x(), bounds.get$width()))) {
      var cos = Math.cos(line.get$rotation()), sin = Math.sin(line.get$rotation()), x1 = $sub(ball.get$x(), line.get$x()), y1 = $sub(ball.get$y(), line.get$y()), y2 = $mul(cos, y1) - $mul(sin, x1);
      var vy1 = $mul(cos, ball.get$vy()) - $mul(sin, ball.get$vx());
