...
chainarray.prototype.run = function() {
  var $0;
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var mouse = Utils.captureMouse(canvas);
  var balls = new Array();
  var numBalls = (5), spring = (0.03), friction = (0.9), gravity = (2);
  while (($0 = numBalls, numBalls = $sub(numBalls, (1)), $0)) {
    balls.add(new Ball.withRadius$ctor((20)));
  }
  function move(ball, targetX, targetY) {
    ball.set$vx(ball.get$vx() + ($mul(($sub(targetX, ball.get$x())), spring)));
    ball.set$vy(ball.get$vy() + ($mul(($sub(targetY, ball.get$y())), spring)));
  ...
  function draw(ballB, i) {
    if ((null == i ? null == ((0)) : i === (0))) {
      move(ballB, mouse.get$x(), mouse.get$y());
      context.moveTo$2(mouse.get$x(), mouse.get$y());
    }
    else {
      var ballA = balls.$index($sub(i, (1)));
      move(ballB, ballA.get$x(), ballA.get$y());
      context.moveTo$2(ballA.get$x(), ballA.get$y());
    }
    context.lineTo(ballB.get$x(), ballB.get$y());
    context.stroke();
    ballB.draw(context);
  }
  ;
