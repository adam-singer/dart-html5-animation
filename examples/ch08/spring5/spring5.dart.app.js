...
spring5.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var mouse = Utils.captureMouse(canvas);
  var ball = new Ball();
  var spring = (0.03), friction = (0.9), gravity = (2), vx = (0), vy = (0);
  function drawFrame(t) {
    ...
    var dx = $sub(mouse.get$x(), ball.get$x()), dy = $sub(mouse.get$y(), ball.get$y()), ax = $mul(dx, spring), ay = $mul(dy, spring);
    vx = $add(vx, ax);
    vy = $add(vy, ay);
    vy = $add(vy, gravity);
    vx = $mul(vx, friction);
