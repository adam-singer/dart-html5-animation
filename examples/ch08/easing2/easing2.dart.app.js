...
easing2.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var mouse = Utils.captureMouse(canvas);
  var ball = new Ball();
  var easing = (0.05), targetX = canvas.get$width() / (2), targetY = canvas.get$height() / (2), isMouseDown = false;
  ...
  function drawFrame(t) {
    ...
    if (!isMouseDown) {
      var vx = $mul(($sub(targetX, ball.get$x())), easing), vy = $mul(($sub(targetY, ball.get$y())), easing);
