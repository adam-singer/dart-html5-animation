...
// ********** Code for top level **************
//  ********** Library triplespring **************
// ********** Code for triplespring **************
function triplespring() {

}
triplespring.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var mouse = Utils.captureMouse(canvas);
  var ball0 = new Ball.withRadius$ctor((20)), ball1 = new Ball.withRadius$ctor((20)), ball2 = new Ball.withRadius$ctor((20)), ball0_dragging = false, ball1_dragging = false, ball2_dragging = false, spring = (0.03), friction = (0.9), springLength = (100), vx = (0), vy = (0);
  ball0.set$x(Math.random() * canvas.get$width());
  ball0.set$y(Math.random() * canvas.get$height());
  ball1.set$x(Math.random() * canvas.get$width());
  ball1.set$y(Math.random() * canvas.get$height());
  ball2.set$x(Math.random() * canvas.get$width());
  ball2.set$y(Math.random() * canvas.get$height());
  canvas.get$on().get$mouseDown().add((function (event) {
    if (Utils.containsPoint(ball0.getBounds(), mouse.get$x(), mouse.get$y())) {
      ball0_dragging = true;
    }
    if (Utils.containsPoint(ball1.getBounds(), mouse.get$x(), mouse.get$y())) {
      ball1_dragging = true;
    }
    if (Utils.containsPoint(ball2.getBounds(), mouse.get$x(), mouse.get$y())) {
      ball2_dragging = true;
    }
  })
  , false);
  canvas.get$on().get$mouseUp().add((function (event) {
  ...
  , false);
  canvas.get$on().get$mouseMove().add((function (event) {
    if (ball0_dragging) {
      ball0.set$x(mouse.get$x());
      ball0.set$y(mouse.get$y());
    }
    if (ball1_dragging) {
      ball1.set$x(mouse.get$x());
      ball1.set$y(mouse.get$y());
    }
    if (ball2_dragging) {
      ball2.set$x(mouse.get$x());
      ball2.set$y(mouse.get$y());
    }
  })
  , false);
  function springTo(ballA, ballB) {
    var dx = $sub(ballB.get$x(), ballA.get$x()), dy = $sub(ballB.get$y(), ballA.get$y()), angle = Math.atan2(dy, dx), targetX = $sub(ballB.get$x(), $mul(Math.cos(angle), springLength)), targetY = $sub(ballB.get$y(), $mul(Math.sin(angle), springLength));
    ballA.set$vx(ballA.get$vx() + ($mul(($sub(targetX, ballA.get$x())), spring)));
    ballA.set$vy(ballA.get$vy() + ($mul(($sub(targetY, ballA.get$y())), spring)));
    ballA.set$vx($mul(ballA.get$vx(), friction));
    ballA.set$vy($mul(ballA.get$vy(), friction));
    ballA.set$x($add(ballA.get$x(), ballA.get$vx()));
    ballA.set$y($add(ballA.get$y(), ballA.get$vy()));
  }
  ;
  function drawFrame(t) {
    ...
    context.beginPath();
    context.moveTo$2(ball0.get$x(), ball0.get$y());
    context.lineTo(ball1.get$x(), ball1.get$y());
    context.lineTo(ball2.get$x(), ball2.get$y());
    context.lineTo(ball0.get$x(), ball0.get$y());
    context.stroke();
    ball0.draw(context);
    ball1.draw(context);
    ball2.draw(context);
...
Ball.prototype.getBounds = function() {
  return new Bound(this.x - this.radius, this.y - this.radius, this.radius * (2), this.radius * (2));
}
// ********** Code for Bound **************
...
Bound.prototype.get$x = function() { return this.x; };
Bound.prototype.set$x = function(value) { return this.x = value; };
Bound.prototype.get$y = function() { return this.y; };
Bound.prototype.set$y = function(value) { return this.y = value; };
Bound.prototype.get$width = function() { return this.width; };
Bound.prototype.set$width = function(value) { return this.width = value; };
Bound.prototype.get$height = function() { return this.height; };
Bound.prototype.set$height = function(value) { return this.height = value; };
// ********** Code for top level **************
function main() {
