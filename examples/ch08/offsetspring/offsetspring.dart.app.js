...
// ********** Code for top level **************
//  ********** Library offsetspring **************
// ********** Code for offsetspring **************
function offsetspring() {

}
offsetspring.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var mouse = Utils.captureMouse(canvas);
  var ball = new Ball();
  var spring = (0.03), friction = (0.9), springLength = (100), vx = (0), vy = (0);
  function drawFrame(t) {
    ...
    var dx = $sub(ball.get$x(), mouse.get$x()), dy = $sub(ball.get$y(), mouse.get$y()), angle = Math.atan2(dy, dx), targetX = $add(mouse.get$x(), $mul(Math.cos(angle), springLength)), targetY = $add(mouse.get$y(), $mul(Math.sin(angle), springLength));
    vx = $add(vx, ($mul(($sub(targetX, ball.get$x())), spring)));
    vy = $add(vy, ($mul(($sub(targetY, ball.get$y())), spring)));
    vx = $mul(vx, friction);
    vy = $mul(vy, friction);
