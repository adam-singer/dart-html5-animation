...
spring3.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var ball = new Ball();
  var spring = (0.03), friction = (0.95), targetX = canvas.get$width() / (2), targetY = canvas.get$height() / (2), vx = (0), vy = (0);
  function drawFrame(t) {
    ...
    var dx = $sub(targetX, ball.get$x()), dy = $sub(targetY, ball.get$y()), ax = $mul(dx, spring), ay = $mul(dy, spring);
    vx = $add(vx, ax);
    vy = $add(vy, ay);
    vx = $mul(vx, friction);
    vy = $mul(vy, friction);
