...
// ********** Code for top level **************
//  ********** Library multispring **************
// ********** Code for multispring **************
function multispring() {

}
multispring.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var mouse = Utils.captureMouse(canvas);
  var ball = new Ball.withRadius$ctor((20));
  var handles = new Array();
  var numHandles = (3), spring = (0.03), friction = (0.9), movingHandle = null;
  for (var i = (0);
   $lt(i, numHandles); i = $add(i, (1))) {
    var handle = new Ball.withRadiusAndColor$ctor((10), "#0000ff");
    handle.set$x(Math.random() * canvas.get$width());
    handle.set$y(Math.random() * canvas.get$height());
    handles.add(handle);
  }
  canvas.get$on().get$mouseDown().add((function (event) {
    handles.forEach$1((function (handle) {
      if (Utils.containsPoint(handle.getBounds(), mouse.get$x(), mouse.get$y())) {
        movingHandle = handle;
      }
    })
    );
  ...
  canvas.get$on().get$mouseMove().add((function (event) {
    if ($ne(movingHandle)) {
      movingHandle.set$x(mouse.get$x());
      movingHandle.set$y(mouse.get$y());
    }
  })
  , false);
  function applyHandle(handle) {
    var dx = $sub(handle.get$x(), ball.get$x()), dy = $sub(handle.get$y(), ball.get$y());
    ball.set$vx(ball.get$vx() + ($mul(dx, spring)));
    ball.set$vy(ball.get$vy() + ($mul(dy, spring)));
  }
  ;
  function drawHandle(handle) {
    context.moveTo$2(ball.get$x(), ball.get$y());
    context.lineTo(handle.get$x(), handle.get$y());
    context.stroke();
    handle.draw(context);
  }
  ;
  function drawFrame(t) {
    ...
    handles.forEach$1(applyHandle);
    ball.set$vx($mul(ball.get$vx(), friction));
    ball.set$vy($mul(ball.get$vy(), friction));
    ball.set$x($add(ball.get$x(), ball.get$vx()));
    ball.set$y($add(ball.get$y(), ball.get$vy()));
    context.beginPath();
    handles.forEach$1(drawHandle);
    ball.draw(context);
  }
  ;
  drawFrame((0));
