...
brownian2.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var dots = [], numDots = (20), friction = (0.95), decay = (0.01), decayColor = Utils.colorToRGB("#ffffff", decay);
  for (var dot, i = (0);
   $lt(i, numDots); i = $add(i, (1))) {
    dot = new Ball.withRadiusAndColor$ctor((1), "#000000");
    dot.set$x(Math.random() * canvas.get$width());
    dot.set$y(Math.random() * canvas.get$height());
    dot.set$vx((0));
    dot.set$vy((0));
    dots.add$1(dot);
  }
  function draw(dot) {
    dot.set$vx(dot.get$vx() + (Math.random() * (0.2) - (0.1)));
    dot.set$vy(dot.get$vy() + (Math.random() * (0.2) - (0.1)));
    dot.set$x($add(dot.get$x(), dot.get$vx()));
    dot.set$y($add(dot.get$y(), dot.get$vy()));
    dot.set$vx($mul(dot.get$vx(), friction));
    dot.set$vy($mul(dot.get$vy(), friction));
    if ($gt(dot.get$x(), canvas.get$width())) {
      dot.set$x((0));
    }
    else if ($lt(dot.get$x(), (0))) {
      dot.set$x(canvas.get$width());
    }
    if ($gt(dot.get$y(), canvas.get$height())) {
      dot.set$y((0));
    }
    else if ($lt(dot.get$y(), (0))) {
      dot.set$y(canvas.get$height());
    }
    dot.draw(context);
  }
  function drawFrame(t) {
    ...
    context.set$fillStyle(decayColor);
