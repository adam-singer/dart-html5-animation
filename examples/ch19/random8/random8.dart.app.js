...
random8.prototype.run = function() {
  var $0;
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var numDots = (300), ball0 = new Ball(), ball1 = new Ball.withRadius$ctor((80));
  ball0.set$x((100));
  ball0.set$y(canvas.get$height() / (2));
  ball1.set$x((300));
  ball1.set$y(canvas.get$height() / (2));
  function detectCollision(x, y, ball) {
    var dx = $sub(x, ball.get$x()), dy = $sub(y, ball.get$y()), dist = Math.sqrt($mul(dx, dx) + $mul(dy, dy));
    return ($lt(dist, ball.get$radius()));
  }
  while (($0 = numDots, numDots = $sub(numDots, (1)), $0)) {
    var x = (0), y = (0);
    while (!detectCollision(x, y, ball0) && !detectCollision(x, y, ball1)) {
      x = Math.random() * canvas.get$width();
      y = Math.random() * canvas.get$height();
    }
    context.set$fillStyle("#000000");
    context.beginPath();
    context.arc(x, y, (2), (0), (6.283185307179586), true);
    context.closePath();
    context.fill();
  }
}
