...
timebased3.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var ball = new Ball(), start_time = new DateImplementation.now$ctor().value, vx = (300), vy = (-300), bounce = (-0.7), gravity = (300);
  ball.set$x(canvas.get$width() * Math.random());
  ball.set$y(canvas.get$height() / (2));
  function getTimer() {
    return ($sub(new DateImplementation.now$ctor().value, start_time));
  }
  var time = getTimer();
  function drawFrame(i) {
    html_get$window().webkitRequestAnimationFrame(drawFrame, canvas);
    context.clearRect((0), (0), canvas.get$width(), canvas.get$height());
    var elapsed = $sub(getTimer(), time), left = (0), right = canvas.get$width(), top = (0), bottom = canvas.get$height();
    time = getTimer();
    vy = $add(vy, ($mul(gravity, elapsed) / (1000)));
    ball.set$x($add(ball.get$x(), ($mul(vx, elapsed) / (1000))));
    ball.set$y($add(ball.get$y(), ($mul(vy, elapsed) / (1000))));
