...
random4.prototype.run = function() {
  var $0;
  ...
  while (($0 = numDots, numDots = $sub(numDots, (1)), $0)) {
    var radius = $mul(Math.sqrt(Math.random()), maxRadius), angle = Math.random() * (6.283185307179586), x = canvas.get$width() / (2) + $mul(Math.cos(angle), radius), y = canvas.get$height() / (2) + $mul(Math.sin(angle), radius);
