...
cosines2.prototype.run = function() {
  ...
  function drawFrame(t) {
    ...
    var dx = $sub(mouse.get$x(), segment1.get$x()), dy = $sub(mouse.get$y(), segment1.get$y()), dist = Math.sqrt($mul(dx, dx) + $mul(dy, dy)), a = (100), b = (100), c = Math.min(dist, $add(a, b)), B = Math.acos(($mul(b, b) - $mul(a, a) - $mul(c, c)) / ($mul($mul((-2), a), c))), C = Math.acos(($mul(c, c) - $mul(a, a) - $mul(b, b)) / ($mul($mul((-2), a), b))), D = Math.atan2(dy, dx), E = $sub($sub(D, B) + (3.141592653589793), C);
    segment1.set$rotation(($sub(D, B)));
