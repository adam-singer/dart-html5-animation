...
playball.prototype.run = function() {
  var $0;
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var ball = new Ball.withRadius$ctor((20));
  var segments = [];
  var numSegments = (5), gravity = (0.5), bounce = (-0.9);
  var target;
  ball.set$vx((10));
  while (($0 = numSegments, numSegments = $sub(numSegments, (1)), $0)) {
    segments.add$1(new Segment((20), (10)));
  }
  segments.$index(segments.get$length() - (1)).set$x(canvas.get$width() / (2));
  segments.$index(segments.get$length() - (1)).set$y(canvas.get$height() / (2));
  function moveBall() {
  ...
  function reach(segment, xpos, ypos) {
    var dx = $sub(xpos, segment.get$x()), dy = $sub(ypos, segment.get$y());
    segment.set$rotation(Math.atan2(dy, dx));
    var w = $sub(segment.getPin().get$x(), segment.get$x()), h = $sub(segment.getPin().get$y(), segment.get$y());
    return new point($sub(xpos, w), $sub(ypos, h));
  }
  function position(segmentA, segmentB) {
    segmentA.set$x(segmentB.getPin().get$x());
    segmentA.set$y(segmentB.getPin().get$y());
  }
  function checkHit() {
    var segment = segments.$index((0)), dx = $sub(segment.getPin().get$x(), ball.get$x()), dy = $sub(segment.getPin().get$y(), ball.get$y()), dist = Math.sqrt($mul(dx, dx) + $mul(dy, dy));
    if ($lt(dist, ball.get$radius())) {
      ball.set$vx(ball.get$vx() + (Math.random() * (2) - (1)));
      ball.set$vy(ball.get$vy() - (1));
    }
  }
  function move(segment, i) {
    if ((null == i ? null != ((0)) : i !== (0))) {
      target = reach(segment, target.x, target.y);
      position(segments.$index($sub(i, (1))), segment);
    }
  }
  ...
  function drawFrame(t) {
    ...
    moveBall();
    target = reach(segments.$index((0)), ball.get$x(), ball.get$y());
    var i = (0);
    segments.forEach$1((function (s) {
      move(s, i);
      i = $add(i, (1));
    })
    );
    checkHit();
    segments.forEach$1(draw);
    ball.draw(context);
  }
  drawFrame((0));
}
...
point.prototype.get$radius = function() { return this.radius; };
point.prototype.set$radius = function(value) { return this.radius = value; };
// ********** Code for Ball **************
...
(function(){
  ...
  var v3/*CSSValueList*/ = 'CSSValueList|WebKitCSSFilterValue|WebKitCSSTransformValue';
  var v4/*DOMTokenList*/ = 'DOMTokenList|DOMSettableTokenList';
