...
pixelmove.prototype.run = function() {
  ...
  function drawFrame(t) {
    ...
    for (var i = (0);
     $lt(i, canvas.get$width()); i = $add(i, (10))) {
      for (var j = (0);
       $lt(j, canvas.get$height()); j = $add(j, (10))) {
        context.set$fillStyle(((($0 = $mod(i, (20))) == null ? null == ((0)) : $0 === (0))) ? "#f00" : (((($1 = $mod(i, (30))) == null ? null == ((0)) : $1 === (0))) ? "#0f0" : "#00f"));
        context.fillRect(i, j, (10), (10));
      }
    }
    var imagedata = context.getImageData((0), (0), canvas.get$width(), canvas.get$height()), pixels = imagedata.get$data();
    for (var y = (0);
     $lt(y, imagedata.get$height()); y = $add(y, (1))) {
      for (var x = (0);
       $lt(x, imagedata.get$width()); x = $add(x, (1))) {
        var dx = $sub(x, mouse.get$x()), dy = $sub(y, mouse.get$y()), dist = Math.sqrt($mul(dx, dx) + $mul(dy, dy)), offset = $mul(($add(x, $mul(y, imagedata.get$width()))), (4)), r = pixels.$index(offset), g = pixels.$index($add(offset, (1))), b = pixels.$index($add(offset, (2)));
        pixels.$setindex(offset, Math.cos($mul(r, dist) * (0.001)) * (256));
        pixels.$setindex($add(offset, (1)), Math.sin($mul(g, dist) * (0.001)) * (256));
        pixels.$setindex($add(offset, (2)), Math.cos($mul(b, dist) * (0.0005)) * (256));
      }
    }
