...
//  ********** Library multicurve3 **************
// ********** Code for multicurve3 **************
function multicurve3() {

}
multicurve3.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var points = new Array();
  var numPoints = (9);
  var ctrlPoint = new point((0), (0));
  var ctrlPoint1 = new point((0), (0));
  for (var i = (0);
   $lt(i, numPoints); i = $add(i, (1))) {
    points.add(new point(Math.random() * canvas.get$width(), Math.random() * canvas.get$height()));
  }
  ctrlPoint1.x = $div(($add(points.$index((0)).get$x(), points.$index($sub(numPoints, (1))).get$x())), (2));
  ctrlPoint1.y = $div(($add(points.$index((0)).get$y(), points.$index($sub(numPoints, (1))).get$y())), (2));
  context.beginPath();
  context.moveTo$2(ctrlPoint1.x, ctrlPoint1.y);
  var i;
  for (i = (0);
   $lt(i, $sub(numPoints, (1))); i = $add(i, (1))) {
    ctrlPoint.x = $div(($add(points.$index(i).get$x(), points.$index($add(i, (1))).get$x())), (2));
    ctrlPoint.y = $div(($add(points.$index(i).get$y(), points.$index($add(i, (1))).get$y())), (2));
    context.quadraticCurveTo(points.$index(i).get$x(), points.$index(i).get$y(), ctrlPoint.x, ctrlPoint.y);
  }
  context.quadraticCurveTo(points.$index(i).get$x(), points.$index(i).get$y(), ctrlPoint1.x, ctrlPoint1.y);
  context.stroke();
}
// ********** Code for point **************
function point(x, y) {
