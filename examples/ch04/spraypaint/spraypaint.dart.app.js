...
$dynamic("get$data").CharacterData = function() {
  return this.data;
}
...
$dynamic("get$data").CompositionEvent = function() {
  return this.data;
}
// ********** Code for ConsoleJs **************
...
$dynamic("get$data").HTMLObjectElement = function() {
  return this.data;
}
...
$dynamic("get$data").ImageData = function() {
  return this.data;
}
...
$dynamic("get$data").MessageEvent = function() {
  return this.data;
}
...
$dynamic("get$data").ProcessingInstruction = function() {
  return this.data;
}
...
$dynamic("get$data").TextEvent = function() {
  return this.data;
}
...
spraypaint.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var log = html_get$document().query("#log");
  var mouse = Utils.captureMouse(canvas);
  var imagedata = context.getImageData((0), (0), canvas.get$width(), canvas.get$height());
  var pixels = imagedata.get$data();
  var brush_size = (25);
  var brush_density = (50);
  var brush_color;
  function onMouseMove(e) {
    for (var i = (0);
     $lt(i, brush_density); i = $add(i, (1))) {
      var angle = Math.random() * (3.141592653589793) * (2), radius = $mul(Math.random(), brush_size), xpos = $bit_or(($add(mouse.get$x(), $mul(Math.cos(angle), radius))), (0)), ypos = $bit_or(($add(mouse.get$y(), $mul(Math.sin(angle), radius))), (0)), offset = $mul(($add(xpos, $mul(ypos, imagedata.get$width()))), (4));
      pixels.$setindex(offset, $sar(brush_color, (16)) & (255));
      pixels.$setindex($add(offset, (1)), $sar(brush_color, (8)) & (255));
      pixels.$setindex($add(offset, (2)), $bit_and(brush_color, (255)));
      pixels.$setindex($add(offset, (3)), (255));
    }
    context.putImageData$3(imagedata, (0), (0));
  }
  ;
  canvas.get$on().get$mouseDown().add((function (e) {
    brush_color = Utils.parseColor(Math.random() * (16777215), true);
    canvas.get$on().get$mouseMove().add(onMouseMove, false);
  })
  , false);
  canvas.get$on().get$mouseUp().add((function (e) {
