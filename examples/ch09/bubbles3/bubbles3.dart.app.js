...
bubbles3.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var balls = new Array();
  var numBalls = (10), bounce = (-0.5), spring = (0.015), gravity = (0.1);
  for (var ball, i = (0);
   $lt(i, numBalls); i = $add(i, (1))) {
    var color = Utils.parseColor(Math.random() * (16777215), false);
    ball = new Ball.withRadiusAndColor$ctor(Math.random() * (30) + (20), color);
  ...
  function checkCollision(ballA, i) {
    for (var ballB, dx, dy, dist, min_dist, j = $add(i, (1));
     $lt(j, numBalls); j = $add(j, (1))) {
      ...
      if ($lt(dist, min_dist)) {
        var tx = $add(ballA.get$x(), $mul($div(dx, dist), min_dist)), ty = $add(ballA.get$y(), $mul($div(dy, dist), min_dist)), ax = $mul(($sub(tx, ballB.get$x())), spring), ay = $mul(($sub(ty, ballB.get$y())), spring);
