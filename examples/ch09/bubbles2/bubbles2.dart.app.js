...
bubbles2.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var centerBall = new Ball.withRadiusAndColor$ctor((100), "#cccccc");
  var balls = new Array();
  var numBalls = (10), bounce = (-0.5), spring = (0.03), gravity = (0.1);
  for (var i = (0);
   $lt(i, numBalls); i = $add(i, (1))) {
    var color = Utils.parseColor(Math.random() * (16777215), false);
    var ball = new Ball.withRadiusAndColor$ctor(Math.random() * (30) + (20), color);
    ball.set$x(Math.random() * canvas.get$width() / (2));
    ball.set$y(Math.random() * canvas.get$height() / (2));
    ball.set$vx(Math.random() * (6) - (3));
    ball.set$vy(Math.random() * (6) - (3));
    balls.add(ball);
  }
  function checkCollision(ballA, i) {
    for (var ballB, dx, dy, dist, min_dist, j = $add(i, (1));
     $lt(j, numBalls); j = $add(j, (1))) {
      ballB = balls.$index(j);
      dx = $sub(ballB.get$x(), ballA.get$x());
      dy = $sub(ballB.get$y(), ballA.get$y());
      dist = Math.sqrt($mul(dx, dx) + $mul(dy, dy));
      min_dist = ballA.get$radius() + ballB.get$radius();
      if ($lt(dist, min_dist)) {
        var angle = Math.atan2(dy, dx), tx = $add(ballA.get$x(), $mul(Math.cos(angle), min_dist)), ty = $add(ballA.get$y(), $mul(Math.sin(angle), min_dist)), ax = $mul(($sub(tx, ballB.get$x())), spring) * (0.5), ay = $mul(($sub(ty, ballB.get$y())), spring) * (0.5);
        ballA.set$vx($sub(ballA.get$vx(), ax));
        ballA.set$vy($sub(ballA.get$vy(), ay));
        ballB.set$vx($add(ballB.get$vx(), ax));
        ballB.set$vy($add(ballB.get$vy(), ay));
      }
    }
  }
  ;
