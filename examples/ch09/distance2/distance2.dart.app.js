...
distance2.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var mouse = Utils.captureMouse(canvas);
  var log = html_get$document().query("#log");
  var ballA = new Ball.withRadius$ctor(Math.random() * (100)), ballB = new Ball.withRadius$ctor(Math.random() * (100));
  ...
  function drawFrame(event) {
    ...
    var dx = $sub(ballB.get$x(), ballA.get$x()), dy = $sub(ballB.get$y(), ballA.get$y()), dist = Math.sqrt($mul(dx, dx) + $mul(dy, dy));
  ...
  ;
  canvas.get$on().get$mouseMove().add(drawFrame, false);
  drawFrame(null);
}
// ********** Code for Ball **************
