...
bubbles1.prototype.run = function() {
  var canvas = html_get$document().query("#canvas");
  var context = canvas.getContext("2d");
  var centerBall = new Ball.withRadiusAndColor$ctor((100), "#cccccc");
  var balls = new Array();
  var numBalls = (10), spring = (0.03), bounce = (-1);
  centerBall.set$x(canvas.get$width() / (2));
  centerBall.set$y(canvas.get$height() / (2));
  for (var i = (0);
   $lt(i, numBalls); i = $add(i, (1))) {
    var color = Utils.parseColor(Math.random() * (16777215), false);
    var ball = new Ball.withRadiusAndColor$ctor(Math.random() * (40) + (5), color);
  ...
  ;
  function draw(ball) {
    var dx = $sub(ball.get$x(), centerBall.get$x()), dy = $sub(ball.get$y(), centerBall.get$y()), dist = Math.sqrt($mul(dx, dx) + $mul(dy, dy)), min_dist = ball.get$radius() + centerBall.get$radius();
    if ($lt(dist, min_dist)) {
      var angle = Math.atan2(dy, dx), tx = $add(centerBall.get$x(), $mul(Math.cos(angle), min_dist)), ty = $add(centerBall.get$y(), $mul(Math.sin(angle), min_dist));
      ball.set$vx(ball.get$vx() + ($mul(($sub(tx, ball.get$x())), spring)));
      ball.set$vy(ball.get$vy() + ($mul(($sub(ty, ball.get$y())), spring)));
    }
    ball.draw(context);
  }
  ;
