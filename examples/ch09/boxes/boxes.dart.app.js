...
boxes.prototype.run = function() {
  ...
  var gravity = (0.2);
  function createBox() {
    var box = new Box.withSize$ctor(Math.random() * (40) + (10), Math.random() * (40) + (10));
    box.set$x(Math.random() * canvas.get$width());
    _boxes.add(box);
    return box;
  }
  ;
  ...
  function drawBox(box) {
    if ((null == activeBox ? null != (box) : activeBox !== box) && Utils.intersects(activeBox, box)) {
      activeBox.set$y($sub(box.get$y(), activeBox.get$height()));
      activeBox = createBox();
    }
    box.draw(context);
  }
  ...
  function drawFrame(t) {
    ...
    activeBox.set$vy($add(activeBox.get$vy(), gravity));
    activeBox.set$y($add(activeBox.get$y(), activeBox.get$vy()));
    if ($gt($add(activeBox.get$y(), activeBox.get$height()), canvas.get$height())) {
      activeBox.set$y($sub(canvas.get$height(), activeBox.get$height()));
      activeBox = createBox();
    }
    _boxes.forEach$1(drawBox);
  }
  ;
  drawFrame((0));
}
// ********** Code for Box **************
Box.withSize$ctor = function(width, height) {
  this.height = height;
  this.width = width;
  this.color = "#ff0000";
  this._defaultValues();
}
Box.withSize$ctor.prototype = Box.prototype;
function Box() {}
Box.prototype.get$x = function() { return this.x; };
Box.prototype.set$x = function(value) { return this.x = value; };
Box.prototype.get$y = function() { return this.y; };
Box.prototype.set$y = function(value) { return this.y = value; };
Box.prototype.get$width = function() { return this.width; };
Box.prototype.set$width = function(value) { return this.width = value; };
Box.prototype.get$height = function() { return this.height; };
Box.prototype.set$height = function(value) { return this.height = value; };
Box.prototype.get$vy = function() { return this.vy; };
Box.prototype.set$vy = function(value) { return this.vy = value; };
Box.prototype.get$lineWidth = function() { return this.lineWidth; };
Box.prototype.set$lineWidth = function(value) { return this.lineWidth = value; };
Box.prototype._defaultValues = function() {
  this.x = (0);
  this.y = (0);
  this.vx = (0);
  this.vy = (0);
...
Box.prototype.draw = function(context) {
  ...
  context.beginPath();
  context.rect$4((0), (0), this.width, this.height);
  context.closePath();
  context.fill();
  if (this.lineWidth > (0)) {
    context.stroke();
